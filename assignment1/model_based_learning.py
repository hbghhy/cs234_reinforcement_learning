...
def update_mdp_model_with_history(counts, rewards, history):
    ...
    for state, action, reward, next_state, done in history:
        rewards[state][action][next_state]=(
            (rewards[state][action][next_state]*counts[state][action][next_state]+reward)/(counts[state][action][next_state]+1)
        )
        counts[state][action][next_state] += 1
        if done:
            counts[next_state][action][state]+=1
    ############################
...
def learn_with_mdp_model(env, num_episodes=5000, gamma=0.95, e=0.8, decay_rate=0.99):
    ...
    for _ in range(num_episodes):
        ...
        for _ in range(400):
            ...
            nextstate, reward, terminal,_=env.step(a)
            history.append([s, a, reward, nextstate, terminal])
            # r=np.random.random()
...
def average_score(env,policy,time=500):
    return np.mean([evaluate_policy(env,policy) for _ in range(time)])



def evaluate_num(env,num_episodes):
    ans=np.zeros(num_episodes.shape)
    for i in tqdm.tqdm(range(num_episodes.shape[0])):
        policy=learn_with_mdp_model(env,num_episodes[i])
        #v,policy=value_iteration(env.P,env.nS,env.nA)
        ans[i]=average_score(env,policy)
    return ans
...
def main():
    ...
    num_episodes = np.arange(1, 2000, 10)
    #num_episodes=np.array([5000])
    out1=evaluate_num(env,num_episodes)
    import matplotlib.pyplot as plt

    plt.plot(num_episodes, out1)
